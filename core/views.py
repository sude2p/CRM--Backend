...
from .models import CustomUser, UserDetail, OrganizationDetail, Role
...
def handle_platform_user_created(event_data):
    ...
    organization = (
        event_data["organizationId"] if "organization" in event_data else None
    )
...
def handle_platform_user_updated(event_data):
    ...
    is_staff = event_data["isStaff"]
    organization = (
        event_data["organizationId"] if "organizationId" in event_data else None
    )
    ...
    try:
        with transaction.atomic():
            custom_user_reference = CustomUser.objects.filter(
                user_ref_id=user_id
            ).first()
...
def handle_organization_created(event_data):
    ...
    try:
        with transaction.atomic():
            organization_reference, created = OrganizationDetail.objects.get_or_create(
                ref_org_id=organization_id,
                defaults={"name": name, "created_by": created_by},
            )
            if created:
                print(
                    f"Organization created: {organization_id}, {user_id}, {email}, {name}"
                )
            else:
                print(
                    f"Organization already exists: {organization_id}, {user_id}, {email}, {name}"
                )
...
def handle_organization_updated(event_data):
    ...
    try:
        print(f"i am here")
        with transaction.atomic():
            reference_organization = OrganizationDetail.objects.get(
                ref_org_id=organization_id
            )
            # Debug: Check if reference_organization is an instance of OrganizationDetail
            print(f"reference_organization is of type: {type(reference_organization)}")
           # Update the fields and save
            if reference_organization:
                ...
                print(
                    f"Organization updated: {organization_id}, {user_id}, {email}, {name}"
                )
            else:
    ...
    if organization_id and user_id:
        try:
            with transaction.atomic():
                # Fetch the single instance of UserDetail and update organization
                user_detail = UserDetail.objects.get(user=user_id)
                user_detail.organization = organization_id
                user_detail.save()
...
def handle_org_user_deleted(event_data):
    ...
    try:
        with transaction.atomic():
            ...
            print(
                f"User with ID {user_id} and associated groups and permissions deleted successfully."
            )
            return JsonResponse(
                {"status": "User and associated data deleted successfully"}, status=200
            )
...
def handle_role_created(event_data):
    event_data = json.loads(event_data.body.decode('utf-8'))  # Load the incoming JSON data
    ...
    """
    user_id = event_data["userId"]
    ref_group_id = event_data["groupId"]
    name = event_data.get("name", "")
    ref_organization_id = event_data["organization"]
    ...
    try:
        with transaction.atomic():
            ...
            try:
                ...
                if user.userprofile.organization != ref_organization_id:
                    return JsonResponse(
                        {
                            "error": f"User with ID {user_id} does not belong to organization {ref_organization_id}"
                        },
                        status=404,
                    )
                        
                    
                # user.userprofile.get(organization=ref_organization_id)
            except ObjectDoesNotExist:
                return JsonResponse(
                    {"error": f"User with ID {user_id} does not exist"}, status=404
                )
            ...
            try:
                organization = OrganizationDetail.objects.get(
                    ref_org_id=ref_organization_id
                )
            except ObjectDoesNotExist:
                return JsonResponse(
                    {
                        "error": f"Organization with ID {ref_organization_id} does not exist"
                    },
                    status=404,
                )

            # Fetch or create the group
            group, created = Role.objects.get_or_create(
                defaults={
                    "name": name,
                    "ref_org_id": ref_organization_id,
            ...
            print(f"Created new role with ref_group_id {ref_group_id}")

            # Check if the user is already in the group
            if not user.groups.filter(
                role__ref_group_id=ref_group_id, role__ref_org_id=ref_organization_id
            ).exists():
            ...
            if existing_permissions != new_permissions:
                ...
                print(
                    f"Updated group ID {ref_group_id} for user ID {user_id} with permissions {permissions}"
                )

        return JsonResponse(
            {"status": "Role and permissions updated successfully"}, status=200
        )
